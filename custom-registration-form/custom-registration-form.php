...
// =================================================================================
function custom_registration_admin_menu() { global $wpdb; $table_name = $wpdb->prefix . 'custom_registrations'; $new_submission_count = $wpdb->get_var("SELECT COUNT(*) FROM $table_name WHERE status = 'New'"); $menu_title = 'Submissions'; if ($new_submission_count > 0) { $menu_title .= ' <span class="awaiting-mod"><span class="pending-count">' . $new_submission_count . '</span></span>'; } add_menu_page('Submissions', $menu_title, 'read', 'custom-registrations', 'custom_registrations_page_content', 'dashicons-list-view', 25); }
...
function custom_registrations_page_content() {
    ...
    <div class="wrap crf-crm-wrapper">
        ...
        <form method="get" class="crf-filters">
            <ul class="subsubsub"><li><a href="?page=custom-registrations&flag=ok" class="<?php echo $current_flag_view == 'ok' ? 'current' : ''; ?>">Active</a> |</li><li><a href="?page=custom-registrations&flag=spam" class="<?php echo $current_flag_view == 'spam' ? 'current' : ''; ?>">Flagged as Spam</a></li></ul>
        ...
        <div class="tablenav top"><div class="tablenav-pages"><span class="displaying-num"><?php echo $total_items; ?> items</span><span class="pagination-links"><?php echo paginate_links(['base' => add_query_arg('paged', '%#%'),'format' => '','prev_text' => __('&laquo;'),'next_text' => __('&raquo;'),'total' => ceil($total_items / $per_page),'current' => $current_page]); ?></span></div></div>
        <table class="wp-list-table widefat fixed striped">
            <thead><tr><th>Name</th><th>Address</th><th>Note</th><th>Images</th><th>Contact</th><th>Tags</th><th>Status</th><th>Submitted</th><th>Actions</th></tr></thead>
            <tbody>
                <?php if (empty($results)) : ?> 
                    <tr><td colspan="9">No submissions found.</td></tr> 
                <?php else : foreach ($results as $row) : ?>
                <tr>
                    <td><strong><?php echo esc_html($row->name); ?></strong></td><td><?php echo esc_html($row->address); ?></td><td><?php echo nl2br(esc_html($row->note)); ?></td><td><div class="crf-image-stack"><?php if (!empty($row->image_url)) { $image_urls = explode(',', $row->image_url); foreach ($image_urls as $url) { if (!empty($url)) printf('<a href="%s" target="_blank"><img src="%s" width="60" height="60" style="object-fit:cover;" alt="Submission Image"/></a>', esc_url($url), esc_url($url)); } } ?></div></td><td><?php echo esc_html($row->phone_number); ?></td><td><?php if(!empty($row->tags)){ $submission_tags = explode(',', $row->tags); foreach ($master_tags as $mt) { if(in_array($mt['name'], $submission_tags)) { printf('<span class="crf-tag" style="background-color:%s;">%s</span>', esc_attr($mt['color']), esc_html($mt['name']));}}} ?></td><td><select class="status-changer" data-id="<?php echo esc_attr($row->id); ?>"><?php foreach ($statuses as $s) { printf('<option value="%s" %s>%s</option>', esc_attr($s), selected($row->status, $s, false), esc_html($s)); } ?></select></td><td><?php echo date("M j, Y", strtotime($row->submitted_at)); ?></td>
                    <td class="action-buttons">
                        ...
                        <?php if ($current_flag_view === 'ok') : ?><button type="button" class="button button-secondary button-small spam-button" data-id="<?php echo esc_attr($row->id); ?>">Spam</button><?php else : ?><button type="button" class="button button-secondary button-small not-spam-button" data-id="<?php echo esc_attr($row->id); ?>">Not Spam</button><?php endif; ?>
                        <?php if (current_user_can('delete_submissions')) : ?>
                            <button type="button" class="button button-danger button-small delete-button" data-id="<?php echo esc_attr($row->id); ?>">Delete</button>
                        <?php endif; ?>
                    </td>
                </tr>
        ...
        <div class="tablenav bottom"><div class="tablenav-pages"><span class="displaying-num"><?php echo $total_items; ?> items</span><span class="pagination-links"><?php echo paginate_links(['base' => add_query_arg('paged', '%#%'),'format' => '','prev_text' => __('&laquo;'),'next_text' => __('&raquo;'),'total' => ceil($total_items / $per_page),'current' => $current_page]); ?></span></div></div>
    </div>
...
function crf_enqueue_admin_assets($hook) { if ($hook != 'toplevel_page_custom-registrations') { return; } $style_path = plugin_dir_path(__FILE__) . 'assets/css/admin-style.css'; if (file_exists($style_path)) { wp_enqueue_style('crf-admin-style', plugin_dir_url(__FILE__) . 'assets/css/admin-style.css', [], filemtime($style_path)); } wp_enqueue_script('crf-admin-notifications', plugin_dir_url(__FILE__) . 'assets/js/admin-notifications.js', ['jquery'], null, true); wp_localize_script('crf-admin-notifications', 'crf_ajax_object', [ 'ajax_url' => admin_url('admin-ajax.php'), 'nonce'    => wp_create_nonce('crf_get_new_submission_count'), ]); }
...
function crf_get_new_submission_count() { check_ajax_referer('crf_get_new_submission_count', 'nonce'); global $wpdb; $table_name = $wpdb->prefix . 'custom_registrations'; $count = $wpdb->get_var("SELECT COUNT(*) FROM $table_name WHERE status = 'New'"); wp_send_json_success($count); }
