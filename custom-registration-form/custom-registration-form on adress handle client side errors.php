...
// =================================================================================
// ... (Activation, Shortcode, Form Handling, and Import/Export code is the same as the previous version)
...
function handle_custom_form_submission() {
    ...
    if ($result === false) { wp_safe_redirect(add_query_arg('submission-status', 'db_error', $current_page_url)); exit; }
...
function custom_registrations_page_content() {
    ...
    <div class="wrap crf-crm-wrapper">
        ...
        <div class="crf-page-actions">
            <a href="<?php echo esc_url(wp_nonce_url(admin_url('admin.php?page=custom-registrations&action=export_csv'), 'crf_export_nonce')); ?>" class="button button-secondary">Export to CSV</a>
            <button type="button" class="button button-secondary" id="import-btn">Import from CSV</button>
        </div>
        ...
        <form method="get" class="crf-filters">
            <ul class="subsubsub">
                <li><a href="?page=custom-registrations&flag=ok" class="<?php echo $current_flag_view == 'ok' ? 'current' : ''; ?>">Active</a> |</li>
                <li><a href="?page=custom-registrations&flag=spam" class="<?php echo $current_flag_view == 'spam' ? 'current' : ''; ?>">Flagged as Spam</a></li>
            </ul>
            <input type="hidden" name="page" value="custom-registrations">
            <input type="hidden" name="flag" value="<?php echo esc_attr($current_flag_view); ?>">
            <p class="search-box">
                <input type="search" name="s" value="<?php echo esc_attr(isset($_GET['s']) ? $_GET['s'] : ''); ?>">
                <input type="submit" class="button" value="Search">
            </p>
            <select name="status"><option value="">All Statuses</option><?php foreach ($statuses as $s) { printf('<option value="%s" %s>%s</option>', esc_attr($s), selected(isset($_GET['status']) ? $_GET['status'] : '', $s, false), esc_html($s)); } ?></select>
            <select name="tag"><option value="">All Tags</option><?php foreach ($master_tags as $t) { printf('<option value="%s" %s>%s</option>', esc_attr($t['name']), selected(isset($_GET['tag']) ? $_GET['tag'] : '', $t['name'], false), esc_html($t['name'])); } ?></select>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="action-buttons">
                        ...
                        <?php else : ?>
                            <button type="button" class="button button-secondary button-small not-spam-button" data-id="<?php echo esc_attr($row->id); ?>">Not Spam</button>
                        <?php endif; ?>
